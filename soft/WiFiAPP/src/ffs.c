...
void ffs_init(void) {
	...
	for (n = 0; n < FAT_SIZE; n++) {
		...
		if ((fat[n].type == TYPE_REMOVED) || (fat[n].type == TYPE_FREE))
			continue;
...
uint32_t ffs_free(void) {
	static const uint8_t n_bits[16] = { 0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2,
			3, 3, 4 };
	uint32_t size = 0;
	uint16_t i;
...
int16_t ffs_create(const char *fname, uint8_t type, uint16_t size) {
	...
	while (s < f_size) {
		...
		while ((s < f_size) && (!(free_sect[s >> 3] & (1 << (s & 0x07)))))
			s++;
		if (s >= f_size)
			break;
		...
		while ((s < f_size) && (s - begin < s_count)
				&& (free_sect[s >> 3] & (1 << (s & 0x07))))
			s++;
		if (s - begin < s_count)
			continue;	// блок слишком короткий
...
void ffs_writeData(uint16_t n, uint16_t offs, const uint8_t *data,
		uint16_t size) {
...
bool ffs_write(const char *fname, uint8_t type, const uint8_t *data,
		uint16_t size) {
	// Создаем файл
...
int16_t ffs_find(const char *fname) {
	...
	for (n = 0; n < FAT_SIZE; n++) {
		if ((fat[n].type == TYPE_REMOVED) || (fat[n].type == TYPE_FREE))
			continue;
		if (ets_memcmp(fat[n].name, tmp, 8) == 0)
			return n;
	}
